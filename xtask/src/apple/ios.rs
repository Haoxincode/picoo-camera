...
pub(crate) fn build_ios(sh: &Shell) -> Result<()> {
    ...
    cmd!(
        sh,
        "xcrun --sdk iphonesimulator clang -target arm64-apple-ios18.0-simulator {smoke_source} -I {include_dir} {simulator_lib} -framework CoreFoundation -framework Network -framework Security -framework SystemConfiguration -o {smoke_binary}"
    )
    .run()?;
