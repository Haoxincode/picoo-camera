...
pub enum WindowsNativeWireMessage {
    /// The Frame Server declares its already-negotiated fixed output layout
    /// before the producer creates renderer resources. Frame rate remains a
    /// consumer-side SampleClock fact and does not alter the shared surface.
    Demand {
        width: u32,
        height: u32,
        output_revision: u64,
        ...
        match self {
            Self::Demand {
                ...
                if !matches!((width, height), (1280, 720) | (1920, 1080)) {
                    return Err(WindowsNativeWireError::Malformed);
                }
                bytes.push(Self::DEMAND);
                put_u32(&mut bytes, width);
                put_u32(&mut bytes, height);
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, WindowsNativeWireError> {
        ...
        match bytes[0] {
            Self::DEMAND => {
                ...
                let output_revision = reader.u64()?;
                reader.finish()?;
                if !matches!((width, height), (1280, 720) | (1920, 1080)) {
                    return Err(WindowsNativeWireError::Malformed);
                }
