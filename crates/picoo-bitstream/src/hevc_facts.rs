...
impl VideoSpsFacts {
    ...
    pub fn parse_hevc(nal: &[u8]) -> Result<Self, BitstreamError> {
        parse_source(nal).map(|(_, facts)| facts)
    }
}

pub(crate) fn parse_source(
    nal: &[u8],
) -> Result<(scuffle_h265::SpsRbsp, VideoSpsFacts), BitstreamError> {
    if nal.len() > 64 * 1024 {
    ...
    let chroma_location = match vui.and_then(|v| v.chroma_loc_info.as_ref()) {
        Some(loc) if loc.top_field == loc.bottom_field && loc.top_field <= 5 => loc.top_field as u8,
        Some(_) => {
            return Err(BitstreamError::Unsupported(
