...
pub trait BitReaderExpGolombExt {
    ...
    fn read_signed_exp_golomb(&mut self) -> io::Result<i64> {
        ...
        if exp_glob.is_multiple_of(2) {
            ...
            i64::try_from(exp_glob / 2 + 1)
                .map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "signed Exp-Golomb overflow"))
        }
    }
}
    ...
    fn read_exp_golomb(&mut self) -> io::Result<u64> {
        ...
        while !self.read_bit()? {
            ...
            if leading_zeros > u64::BITS {
                return Err(io::Error::new(io::ErrorKind::InvalidData, "Exp-Golomb prefix overflow"));
            }
        }

        let mut suffix = 0_u64;
        for _ in 0..leading_zeros {
            suffix = (suffix << 1) | u64::from(self.read_bit()?);
        }
        // A 64-zero prefix represents u64::MAX only when its suffix is zero.
        let base = if leading_zeros == u64::BITS {
            u64::MAX
        } else {
            (1_u64 << leading_zeros) - 1
        };
        base.checked_add(suffix)
            .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "Exp-Golomb value overflow"))
    }
}
