...
impl VuiParameters {
    ...
    pub(crate) fn parse<R: io::Read>(
        ...
        if default_display_window_flag {
            ...
            let add = |left: u64, right: u64| left.checked_add(right)
                .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "display window overflow"));
            super::bounds::remaining(pic_width_in_luma_samples.get(),
                add(conformance_window.conf_win_left_offset, def_disp_win_left_offset)?,
                add(conformance_window.conf_win_right_offset, def_disp_win_right_offset)?, sub_width_c)?;
            super::bounds::remaining(pic_height_in_luma_samples.get(),
                add(conformance_window.conf_win_top_offset, def_disp_win_top_offset)?,
                add(conformance_window.conf_win_bottom_offset, def_disp_win_bottom_offset)?, sub_height_c)?;
