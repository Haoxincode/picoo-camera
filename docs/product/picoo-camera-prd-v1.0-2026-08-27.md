...
酒店、机场、会议场地等访客 Wi-Fi 可能启用客户端隔离（AP Isolation）或把终端划分到互相不可访问的 VLAN。此时手机与电脑即使显示连接到同一个 Wi-Fi，mDNS 自动发现和手动 `IP:4433` 直连都可能失败；这不是配对或视频协议错误，当前局域网直连架构也不能绕过网络侧隔离。

遇到这种情况，应让电脑开启个人热点，再让手机连接电脑热点，随后在 Picoo Camera 中重新发现或连接电脑。也可以使用旅行路由器建立手机与电脑共同加入的私有局域网。产品不通过云中继解决该场景，电脑热点或私有局域网是现场连接兜底。
...
桌面端的视觉与信息架构以 [REQ-PICOO-UI-0001](../design-specs/requirements/req-picoo-ui-0001-native-camera-and-desktop-gpui-acceptance.md) 与 [ARCH-PICOO-UI-002](../design-specs/architecture/0010-cross-platform-design-system-boundary.md) 为准。共享 token 映射为 GPUI 的 `rem` 比例、组件语义尺寸和 Picoo 明暗主题，首次启动默认使用亮色主题，用户仍可在 Sidebar 中切换深色主题；不引入 CSS、WebView 或浏览器运行时，也不以像素复刻为由绕过 gpui-component 已有的键盘、焦点、滚动与弹窗行为。

桌面 Receiver 默认窗口与最小窗口均为 1440×900，不记忆用户上次调整的窗口尺寸。连接页在 Sidebar 之后使用全部可用宽度，不另设内容宽度上限。待机态左侧配对主区域消费全部剩余宽度，网络状态在同一主卡内部底部横向摊平，不另建带圆角或外边框的长卡；右侧只保留 21–24rem 的设备与连接卡，且不随窗口额外拉宽。「自动接受可信设备」固定在卡片底栏，可信设备列表独立持有纵向溢出。“开始使用”旁的手机→无线→电脑真机 SVG 拓扑作为完整插图按可用宽度等比缩放并设置舒适上限，在最小窗口不得越过所属卡片边框。可信设备行使用中性手机机框 SVG 建立三行行高，依次完整显示设备名称、人类可读的最近连接时间（今天 / 昨天 / N 天前）和身份指纹，等待状态与透明底红色 X 操作使用稳定尾部车道；删除按钮采用标准中号组件，整框均可命中，点击后直接异步撤销信任并反馈成功或失败，不打开确认框；底栏“重置全部配对”同样直接提交，成功后断开当前连接并反馈结果，不打开确认框。横向网络状态在默认与最小窗口下完整显示图标、文字和状态符号，连接工作区自身不滚动。

进入 Live 后整个右侧设备栏退出，工作区只保留完整单行顶栏与实时预览。顶栏左侧展示设备名、实测帧率、链路延迟、接收码率和网络质量；右侧为分辨率选择、「连接详情」以及带图标和文字的镜像、切换、修复、断开操作。帧率、延迟、码率和网络质量各自使用独立圆角方块；分辨率选择与连接详情使用与镜头控制相同的 outline 按钮；顶栏与预览之间用细分隔线分开；不得用隐藏、折叠、缩短文案、icon-only 或收入详情的方式做紧凑降级。连接详情只承载身份、视频规格、虚拟摄像头、可观测丢片、抖动和播放缓冲等深入信息。顶栏下方全部剩余空间交给 16:9 视频视口，使其成为页面最大的视觉对象；画面内部只允许左上角一处深色 `Live` 胶囊，不叠加设备名、规格、虚拟摄像头状态或等待提示。断开操作使用透明底红色 X 与文字，并保留确认框。
...
控制消息使用 Protobuf，并通过 Rust prost 生成协议类型。每个可靠控制帧只解码一次
`ControlEnvelope`，其中 oneof payload 标识消息类型，message ID 与 connection generation 隔离
重复、越序和旧连接事件；禁止依次尝试解码多个消息类型或使用 magic discriminator。
...
- `ControlEnvelope` 单次解码、消息分派与阶段约束
- 视频分包
- 乱序重组
- 重复包
- 缺失包
