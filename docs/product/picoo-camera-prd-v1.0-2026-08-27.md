...
进入 Live 后整个右侧设备栏退出，工作区只保留完整单行顶栏与实时预览。顶栏常驻展示设备名、唯一一处绿色 `Live`、分辨率选择、实测帧率、链路延迟、接收码率、网络质量、「连接详情」以及带图标和文字的镜像、切换、修复、断开操作；不得用隐藏、折叠、缩短文案、icon-only 或收入详情的方式做紧凑降级。连接详情只承载身份、视频规格、虚拟摄像头、可观测丢片、抖动和播放缓冲等深入信息。顶栏下方全部剩余空间交给 16:9 视频视口，使其成为页面最大的视觉对象；画面内部不叠加设备名、规格、虚拟摄像头状态、等待提示或 `LIVE`。断开操作使用透明底红色 X 与文字，并保留确认框。
