...
- [U04] 发布元数据：gpui-pre-http-client 0.3.6 / zed snapshot。
...
[R17]: https://crates.io/crates/gpui-kit/0.6.6 "GPUI Kit 锁定版本 0.6.6"
...
[U04]: https://docs.rs/crate/gpui-pre-http-client/0.3.6 "发布元数据：gpui-pre-http-client 0.3.6"
...
## 2026-09-07 已确认补充：镜头切换的自动适配

用户切换前后镜头时，自动选择目标镜头和接收端共同支持的上限，而非因原格式不可用而拒绝翻转。优先保留当前codec，其次最高分辨率，再选该分辨率最高fps；没有当前codec候选时才切换codec。切回能力更高的镜头重新选取上限。镜头和完整格式作为一次源事务提交；无共同候选或实际准备失败恢复旧事实。此规则仅适用于明确镜头切换，不改变画质菜单所选完整格式或运行中禁止静默降级的规则。

### 2026-09-07 已确认补充：连接优先

先完成连接和配对，再根据当前镜头及Receiver的完整能力准备媒体。默认请求可用时保留，不可用时为初始媒体自动选择可用上限。相机准备未知、失败或没有共同格式都不阻断网络连接，界面分别呈现连接状态与媒体状态；该策略不改变运行中显式画质选择的契约。
