...
### 2026-09-07：完整已提交源格式事实

- REQ-PICOO-MEDIA-055：Core 只在原生 AU 接纳或事务提交后记录完整 SourceFormat；准备请求和失败不改写旧事实。断连保留最后已提交格式作为恢复事实，不表示正在传输；UI快照接线仍待完成。
- 已绑定世代即使拥有两种合法 Decoder offers，也不能通过普通回调改变 codec/fps/尺寸；拒绝前不暂存配置、不发送控制消息。码率、镜像等描述属性仍能更新，合法显式事务可切换。
- Sender77项测试、Clippy、格式与文档检查通过。新增合同包含AVC→HEVC、HEVC30→60、准备失败回滚、合法切换、断连保留。
- bc63498 的CI 34082811315中Android job在其重复运行workspace测试时，Receiver软件诊断丢包合同只观察到15帧，未达到20帧；正在排查，尚未把该轮标记成功。

### 2026-09-07：修正软件诊断丢包输入

- CI暴露的REQ-PICOO-SESSION-006诊断合同原先把同一个IDR反复发送，却按五帧间隔标记关键帧；还以远高于声明30fps的速度突发发送。旧计数会重复统计同一张latest帧，不能作为不同解码帧数量证据。
- 复用已有OpenH264测试依赖生成120张真实IDR/P预测序列，每五帧闭合刷新，禁用编码跳帧，按声明30fps发送并给出对应PTS。统计唯一源帧身份；保持5%注入丢包、至少20帧和60个发送周期无进展门槛，不修改生产调度器。
- Linux ARM64独立丢包合同通过；完整Receiver121项（另2忽略）、Sender77项和相关Clippy通过。此软件诊断不替代原生1080p60与热稳态验收。

### 2026-09-07：已提交格式跨平台快照
...
### 2026-09-07：Android完整源格式选择

- REQ-PICOO-MEDIA-057：设置与直播共用纯Compose SourceFormatSheet；仅显示当前完整准备交集，显式选择AVC/HEVC × 720/1080 × 30/60。默认意图为AVC1080p60；旧高度偏好不迁移，删除固定30fps文案、分辨率循环和标签反解析。
- SenderSourceSelection归ViewModel作用域所有，后台复用平台准备查询并按镜头/显示方向取消旧工作。持久化默认请求、Core最后提交事实、未知/空能力分别表达；连接/重连中没有新Receiver证据时，不显示仅本地支持的候选。
- 新连接等待相机权限和有效能力，再建立完整源事务；等待期间阻止旧预览AU自行绑定。不可用的默认请求保留连接和完整候选供用户明确选择，不自动降级；原生Error不由UI轮询无限重启。选择成功仅发起事务，HUD仍显示Core已提交格式，尚无事实时显示等待提交。
...
### 2026-09-07：iOS镜头自动适配对齐

- REQ-PICOO-MEDIA-061：复用原生准备交集，按当前codec、分辨率、fps顺序选择目标镜头完整上限。等待查询后核对镜头、源格式、epoch及直播状态；原生切换同时应用目标格式和按目标高度派生的初始码率，失败恢复先前完整格式与码率。
- Swift模拟器套件通过，新增上限选择合同验证1080p30优先于720p60、保持codec、不凭字段组合出1080p60、回后摄恢复更高上限及空交集。日志/tmp/picoo-ios-camera-ceiling-final-test.log；测试包含最终Swift编译。iPhone原生前后镜头实际格式与fps仍未验证，Android真机证据不可替代。

### 2026-09-07：连接优先与预览空档恢复

- 用户已确认Android正式GUI镜头切换成功，随后反馈重连预览停滞以及默认格式阻止连接。REQ-PICOO-MEDIA-062将连接/配对与媒体准备分离：默认完整候选不可用则采用共同上限，无候选保留连接；Android与iOS使用相同规则。Android构建、JVM与iOS模拟器测试通过，Android新包安装后用户确认连接成功，但报告Mac预览空白。
- REQ-PICOO-MEDIA-063回归测试在旧实现失败：150ms源空档使显示需求被100ms新鲜度门槛丢弃。现在复用GPUI request_animation_frame维持可见显示回调，每个显示需求只消费一次，去掉基于绘制时间的可见性猜测。桌面71项测试通过，含源空档与单次消费。该缺陷已有回归证据，但本次现场空白是否仅由此导致仍待新包验证，不以单元测试代替现场恢复、隐藏/恢复验收。

- 新Mac包正常GUI启动，钥匙串授权后UDP4433监听并连接小米15。现场接收日志decoded_frames连续338→398→459→519→579，约60fps，窗口内重组丢帧和恢复计数为零；用户明确确认电脑预览已恢复。没有保存相机画面。隐藏/恢复及多次重连压力测试仍不由本次确认替代。

### 2026-09-07：iOS方向事务与恢复
...
- REQ-PICOO-MEDIA-071串联AU通道、独立重排、原生分段与bundle。原生对象在线程内创建和销毁；接收侧非阻塞offer/stop，原子状态与OnceLock结果，不join。全进程一个工作者直到实际退出才释放名额；等待IDR最多5秒，关键帧请求合并。
- 24项macOS隔离测试和Clippy通过，线程集成验证乱序停止排空、原生完成段、第二工作者拒绝、启动路径失败与真实5秒Arming超时。Receiver接线、UI、原生I/O卡死外部监测和cancel后partial留存仍待完成；操作系统I/O或原生cancel不由Arming期限保证终止。

### 2026-09-07：Receiver录像入口与明确尾部缺口

- REQ-PICOO-MEDIA-072：macOS Receiver在当前配置完整AU进入live恢复/调度前clone压缩Bytes提交；重组期限/容量丢失及断连以同一有界事件通道通知录像，明确尾部丢失不能等下一AU推断。录制关键帧请求直接发EncoderCommand，不触发Decoder恢复。开始需已授权视频配置，停止只关生产者；结果留在worker供上层读取。
- Receiver117项通过、2项原有忽略；录像24项通过，含直播清队列后录制仍保存乱序的两张完整AU，以及尾部gap后停止得到HasGaps；两crate Clippy通过。均为合成输入，没有启用真实手机录像。
- 新字段/接口仅接入Core，桌面命令/状态/按钮仍未接入；跨QUIC通道早到、尚未配置的完整AU目前沿用Receiver已有暂存策略，必须继续独立梳理，不能宣布完整原码流录像验收。703a2c0的CI34103669887全平台成功；f2a2389的新CI34105638062运行中，当前入口变更尚未推送。

### 2026-09-08：录像配置晚到门禁

- REQ-PICOO-MEDIA-073将录制完整AU入口前移到重组完成、live只留单IDR的选择之前。独立配置等待队列保留全部完整AU，绑定连接/epoch，最多16项/250ms；已知配置不可被新配置改贴，缺配置停止或超限明确终止录像。
- 录像26项、Receiver117项通过（另2项原有忽略），两crate Clippy及文档检查通过。覆盖多张未来世代完整AU、错误连接配置不能释放、乱序身份保留、期限不重置、容量以及live恢复不清除录制数据。测试使用合成fixture。
- f2a2389的CI34105638062已全平台通过。本次门禁及60dd676的Receiver接线待新CI；桌面产品入口、异常原生取消留存、全局预算与Windows录制仍未完成。

### 2026-09-08：Apple取消后partial留存

- REQ-PICOO-MEDIA-074通过原生创建时的只读文件句柄保留已写入内容，在cancel完成后无覆盖恢复被删除路径。已取消/失败writer不再调用原生finishWriting，避免无效状态调用；显式恢复错误返回调用方，Drop尽力清理。
- 录像28项与Clippy通过，包括AVC/HEVC生产mux取消后的字节一致、重复清理、已有路径不覆盖和目录失效报错。只证明字节留存，未最终化文件仍不声明可播放；原生I/O阻塞监测仍待补齐。24ed6cf已推送，CI34170528193运行中。

### 2026-09-08：桌面原码流录像入口

- REQ-PICOO-MEDIA-075接入原生目录选择、异步开始/停止、Live 顶栏状态和结果目录定位。UI只保存未回复命令和操作错误；最终状态来自Receiver。长错误通过标准详情弹窗呈现，页面切换不改变录制生命周期。Windows原生mux未接入时不提供录像入口。
- 本地桌面73项、录像28项、Receiver117项通过（另2项原有忽略），补充重复开始拒绝、重复停止与最终收尾状态回归单项通过；桌面all-targets Clippy、文档检查、macOS release构建和Camera Extension打包通过。测试均用合成fixture，未录制手机画面。
- 新包已通过正常GUI方式启动；仅给本地测试包补ad-hoc签名。线程采样显示等待系统Keychain授权，尚未完成实际按钮/目录选择窗口验收，不以单元测试替代该证据。34170528193已全平台成功；本批仍需新CI。

### 2026-09-08：录制工作者停滞观察

- REQ-PICOO-MEDIA-076由Receiver独立读取工作者原子推进时间，15秒未推进显示无响应；观察不获取工作者锁、不join、不释放仍在原生使用的资源，也不虚构Failed/Complete。最终结果优先于瞬时状态，原生最终化与结果发布之间呈现收尾；失败入口不再接收AU或发RAP请求。桌面录制快照独立为概念模块，runtime主文件降至766行。
- 本地录像29项、Receiver117项（另2项原有忽略）、桌面73项通过；模拟阻塞与虚拟时间恢复测试、终止后的关键帧抑制及UI状态覆盖。桌面all-targets Clippy与文档检查通过。本能力不等于任意磁盘卡死可强制清理，真实存储故障和实际UI仍待验证。22fc5a8已推，CI34171573353运行中，未覆盖本批。

### 2026-09-08：录制跨阶段字节预算

- REQ-PICOO-MEDIA-077将配置等待、通道、重排和正在写入的压缩输入合并到进程16MiB上限。AU预留载荷与最大64KiB配置额度，凭据随所有权移动且不可clone；返回/丢弃释放，超额使录像失败。配置晚到也核对记录尺寸，外部不能直接构造未计费输入字段。该限额不包含平台mux内部池或网络重组。
- 录像31项、Receiver117项（另2项原有忽略）通过，覆盖真实通道→重排→批量写入持有、失败排空释放、跨线程释放及计数溢出；相关all-targets Clippy与文档检查通过。录制原生I/O停滞检测提交9a0fcf0尚未推送，等待22fc5a8的CI终态后一起推送。

### 2026-09-08：录制AU绝对提交期限

- REQ-PICOO-MEDIA-078为完整AU绑定首次接纳后的2秒单调时钟期限，穿过配置等待、通道、重排和停止批次，不在阶段切换时重新计时。原生分段/准备后和Busy重试前检查，写入返回后也检查；失败保留此前有效前缀，manifest记录具体的输入超期原因。最终化本身的系统阻塞仍由独立观察呈现，不承诺强制中断。
- 录像33项、Receiver117项（另2项原有忽略）通过，涵盖配置等待不重置、旧输入不能借新队列延长、超期后的原生有效前缀与Failed原因；相关all-targets Clippy和文档检查通过。CI34171573353尚在运行，当前本地提交将待其终态后推送。

### 2026-09-08：Windows原生mux研究入口
...
- 桌面在连接成功后的 Live 顶栏以「录像」按钮呈现两个录像入口，使用独立pending/loading与操作错误。处理后格式菜单按源帧率门控，不支持的源规格在开始前禁用并解释原因；Windows未接入处理后硬编owner时不显示该能力。
