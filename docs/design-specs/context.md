...
| `Picoo Camera Protocol (PCP/1)` | Sender 与 Receiver 之间的控制与视频传输协议，QUIC ALPN 为 `picoocam/1`。 | 控制消息走可靠 Stream；视频片段走 QUIC Datagram。 |
...
- Linux 可运行同一套 GPUI 壳，用来验证桌面功能与 UI；不构成产品 Receiver，也不提供虚拟摄像头。
