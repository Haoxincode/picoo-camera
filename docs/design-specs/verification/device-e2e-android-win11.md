...
| VCam 有设备但仍是 Disconnected 占位 | 桌面是否在 Streaming；`picoo-vcam-ring-reader.exe` 是否附着 `%ProgramData%\Picoo Camera\frame-ring-*.bin` 且 `seq` 递增；`icacls "$env:ProgramData\Picoo Camera"` 是否包含 `LOCAL SERVICE` / `Users` 继承读写 ACL，以及 Medium integrity（`Mandatory Label\Medium Mandatory Level`）；桌面诊断若出现 `拒绝访问 (os error 5)`，用「安装或修复」或重装 MSI 后重启 Picoo Camera |
