...
用户无法通过自动发现找到电脑，但仍希望通过输入桌面端显示的局域网 `IP:端口` 到达 Receiver，再核对两端显示的六位配对短码以建立可信连接。

## 完成标准

- 用户能在 Android 或 iPhone 与 Windows 或 macOS 的任意一种组合上完成从安装到会议选用的完整流程。
- 会议软件中能看到并选用 `Picoo Camera`，画面来自当前连接的手机摄像头。
- 用户能在手机端切换前后摄像头、480p/720p/1080p 和镜像，并在数秒内恢复可用画面。
...
- 480p30、720p30 与 1080p30 视频规格。
- 前后摄像头切换、镜像与曝光补偿。
- 虚拟摄像头注册与会议软件兼容。
- 自动发现、手动 IP 直连、配对短码核对与重连。
...
- [PUC-008](../product/puc-008-connect-with-code-or-ip.md)：通过配对短码核对或 IP 地址直连电脑
