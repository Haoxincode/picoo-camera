...
传输建立后，Sender 显示直播页：本机摄像头预览、连接质量、当前分辨率与帧率、目标 Receiver 名称，以及前后摄像头、480p/720p/1080p、镜像和断开控制。电脑端输出固定为横向 16:9；Sender 的四角构图框必须准确表达 Receiver/Virtual Camera 最终可见范围，而不是与真实输出无关的装饰参考线。

手机横持时，16:9 输出范围应尽量利用完整取景画面。手机竖持时，Sender 仍保持电脑端横向 16:9 输出，只保留直立画面中央的横向区域；构图框外内容不进入电脑端输出，并提示用户横屏使用可获得更完整的画面。手机方向变化不得让电脑端画面侧躺，也不得把输出契约切换为 9:16。
...
- 手机横持与竖持时，电脑端均得到直立的横向 16:9 画面；Sender 构图框与实际输出裁切一致。
