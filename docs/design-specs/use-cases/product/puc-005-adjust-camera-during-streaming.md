...
传输建立后，Sender 显示直播页：全屏本机相机取景器、连接质量、当前分辨率与帧率、目标 Receiver 名称，以及前后摄像头、480p/720p/1080p、镜像和断开控制。电脑端输出固定为横向 16:9；Sender 不叠加容易误解的四角构图框。

手机取景器无论横竖持都使用 center-cover 铺满屏幕，不因比例差异产生黑边。手机竖持时，Sender 仍保持电脑端横向 16:9 输出并在编码前只保留直立画面中央的横向区域；本机取景器用于控制相机，不声称与电脑端逐像素同构。手机方向变化不得让电脑端画面侧躺，也不得把输出契约切换为 9:16。
...
用户切换 480p、720p 或 1080p 时，编码器按协商能力回退；Receiver 自适应调整解码并更新 LatestFrameStore。用户开启远端镜像时，Receiver 在输出到虚拟摄像头和桌面预览的路径上应用镜像；本机预览镜像与远端输出镜像相互独立——默认前置摄像头本机预览镜像，传输到会议软件的画面不镜像。
...
- 手机横持与竖持时，电脑端均得到直立的横向 16:9 画面；Sender 本机取景器铺满且无构图参考框。
