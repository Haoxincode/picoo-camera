...
桌面原码流录像入口属于固定工作区，切换页面不改变录制生命周期。保存位置使用平台原生文件夹选择器，取消不创建录像；开始/停止走Receiver命令队列，接纳回复不代表文件已保存。状态来自录制owner快照，区分准备、录制、收尾和最终的完整/缺口/失败结果；保留目录可由系统打开。UI只持有尚未回复的命令与操作错误，不执行媒体或磁盘工作。
