...
iOS的VideoSourceFormat贯穿相机请求、VideoToolbox配置和Core事务；默认意图为AVC1080p60，最后已提交事实独立于请求。候选由同一AVFoundation format的尺寸/fps与实际要求硬件的VideoToolbox准备会话求交，临时会话顺序创建并立即释放；复用生产属性配置，不引入第二套编码器规则或依赖。准备不等于首AU实际记录已准入，也不代表持续帧率。SwiftUI用既有Picker/Menu表达完整候选，不组合独立最大值；不可用默认保留连接供明确选择。恢复缓存保存完整codec/尺寸/fps，AU必须匹配完整格式和原生世代才能完成事务。
