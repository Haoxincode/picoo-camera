...
原码流录制工作者独占原生段、短时重排和bundle。Receiver仅提交完整AU、读取原子状态/一次性结果及消费合并的关键帧请求；停止通过关闭生产者排空，不能在Receiver或UI等待线程join。全进程最多一个工作者，旧线程真正退出后才释放名额；系统I/O迟滞不能通过不断创建新线程绕过上限。等待可写IDR设独立期限，编码/磁盘错误只终止录像。
...
录制工作者的推进时间由接收侧独立观察，不能由同一被阻塞的原生调用负责宣布超时。超过观察期限时呈现写入无响应；该状态只说明线程未推进，不等于文件失败或原生对象可释放。最终文件结果仍只由录制owner发布，线程实际退出前保留资源名额，UI不强杀线程或接管文件。已终止入口不再请求关键帧。
