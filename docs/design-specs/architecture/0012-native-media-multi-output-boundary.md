...
桌面将原码流与处理后录像呈现为两个独立消费者，分别开始、停止、观察结果并保留各自目录；一个模式的活跃状态不代替或禁止另一个模式。处理后录像开始时显式选择AVC/HEVC与不高于当前源的30/60fps，输出尺寸固定为开始时的当前720p/1080p源规格，不随预览窗口、页面或后续UI布局变化。当前无效果编辑器时scene revision固定为身份场景零；不能借用传输配置revision虚构效果变更。Windows尚无处理后硬编owner时不显示该入口，不能用原码流成功伪装能力可用。
