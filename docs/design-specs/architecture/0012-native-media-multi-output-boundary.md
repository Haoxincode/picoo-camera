...
用户以手机作为电脑摄像头，同时预览和保存视频。各输出共享源内容与时间身份，但不能因会议客户端、窗口或磁盘变慢而相互阻塞。架构优先长期维护简单，协议、FFI、配置和 IPC 直接遵循当前契约。ALPN 固定为 `picoocam`，不引入版本号或版本协商。流/资源 generation 仅用于生命周期安全，不是软件版本。
...
Windows 的共享句柄和 CPU 输出 ring 是不同的交接后端；macOS 两种准备方式共用合法 CMIO sink/source 交接。Mac Camera Extension 在同一设备注册 source 与 sink，sink 只授权 Picoo Host signing ID；Host 以精确 legacy UID、output scope 和 stream direction 找到唯一 sink，使用系统 `CMSimpleQueue` 提交由 `CMIOSampleBufferCreateForImageBuffer` 创建的图像 sample。队列固定三槽且只允许一个 Producer；每个 sample 保留至系统以同一 token 通知移除，callback 同时核对不复用的 owner generation，防止旧连接迟到回调释放新连接碰巧复用地址的 sample。stop 顺序为停流、注销 callback、确认 reset queue 成功后清理 owner；任一步失败则保留 owner/in-flight 引用并停止重连。source 客户端未取流时扩展不得持续消费 sink 或触发 Host pixel demand；开始取流后只缓存尺寸/格式与共享选择一致的最后图像，由 source 自身有理数 SampleClock 重复内容并生成新时间戳，无图像时输出黑帧。source 格式变化清除缓存并通知 sink；Host sink sample 使用系统 host clock，不能把源 PTS、占位零点或配置重置带入 CMIO output 时间线。

macOS `GpuNative` 直接把统一 Apple renderer 的已完成 IOSurface-backed NV12 目标交给 CMIO；显式诊断 `CpuBridge` 先由 `CpuExporter` 读取相同目标，再逐行复制到独立三槽 IOSurface-backed pool。两者都在固定横向 720p/1080p 布局内完成 contain、旋转、镜像与色彩处理，不允许 Extension 再做第二次缩放。Receiver VCam worker 为 latest-only 单槽：新请求替换未开始的旧请求；准备完成后必须在同一状态锁内复核 output revision 再入队，waiting/reconnecting 占位因此能线性化淘汰未入队 live。队列已满只保留当前最新 prepared 图像并有界重试，不建立历史队列。必须分别验收两后端真实系统 sample，不能用 CPU 成功代替 native 完成。

依赖选择优先平台官方能力：CMIO sink/source、Hardware object property API、`CMSimpleQueue`、CoreMedia/CoreVideo/IOSurface 都来自 Apple SDK；`objc2` 用于已有 CoreFoundation/CoreMedia/CoreVideo 所有权类型。当前成熟 Rust 绑定未覆盖 CMIO Hardware output queue，Host 只在单一私有模块手写所需 C ABI 常量与函数，不复制 Camera Extension、队列或 sample 实现。AVFoundation 没有向 Camera Extension sink 提交 output sample 的等价边界，App Group mmap、自定义跨进程原子 ABI 与全局 IOSurface lookup 均不采用。

| 候选 | 当前版本与许可 | 平台、性能与体积判断 | 结论 |
| --- | --- | --- | --- |
| Apple CoreMediaIO/CoreMedia/CoreVideo/IOSurface | macOS 15 产品基线随系统 SDK；系统框架，无仓库包版本 | 仅 macOS；系统管理跨进程队列和 IOSurface sample，不增加第三方运行时、像素 IPC 或额外堆复制 | 采用，Extension 与 Host 都直接使用官方契约 |
| `objc2` / `objc2-core-foundation` / `objc2-core-media` / `objc2-core-video` | lockfile 为 `objc2 0.6.4` 与 framework crates `0.3.2`；MSRV 1.71；MIT 或 Zlib/Apache-2.0/MIT | 仅在 macOS feature 下复用仓库已有的 CF、sample 与 pixel buffer 所有权封装；`cargo tree` 确认没有因本边界引入新的非 Apple 包 | 采用已有依赖，不另包一层通用平台抽象 |
| 自写完整 CMIO/IPC 库 | 不适用 | 会复制系统队列、sample 与跨进程生命周期，扩大 unsafe、内存和维护面 | 不采用；仅为当前生成绑定缺失的 Hardware property、stream start/stop 与 queue callback 保留窄 C ABI 模块 |

Windows Frame Server 是虚拟摄像头固定输出布局与 SampleClock 的 owner。GpuNative 会话必须先由 Consumer 声明已经协商的 1280×720 或 1920×1080 NV12 布局和 output revision，Producer 才能在源 D3D11 device 上建立同尺寸目标；源的可见裁剪、旋转与镜像由 renderer 在该固定横向目标内 contain，不能通过交换目标宽高把竖屏源伪装成另一种 MF 类型。Producer 保留最新不可变源帧，并按 Consumer 的 sample 释放节奏重复准备，重复内容保持源身份不变，时间戳只由 Consumer SampleClock 推进，指标按 source identity 区分 Fresh/Cached。尺寸或帧率事务原子推进 output revision 并使 native session 失效；会话另有 checked revision，旧会话的准备结果、传输失败及 tracked-sample 最终引用释放只能确认原资源，不能回写或清空新会话。Control channel 必须双向认证：Producer 只接受 Local Service Frame Server，Consumer 只接受与已加载 Source DLL 同一受保护安装目录的 `picoo-desktop.exe`，未认证 peer 不得提供或接管 HANDLE。Offer 成功写入 control channel 后 duplicated HANDLE 数值已经暴露给 Consumer，Producer 必须立即停止远程 HANDLE 回收，仅把 surface lease 保留到 Released；Consumer 对已认证 Offer 先拒绝零值、伪句柄和无效句柄，再以 RAII 在所有成功/拒绝路径关闭一次目标进程 HANDLE，只有发布前失败才允许 Producer `DUPLICATE_CLOSE_SOURCE`，避免泄漏和与句柄值复用竞争。MF Source 直接把系统 DXGI surface buffer 放入 sample，不能以只实现 CPU buffer 接口的包装隐藏 `IMFDXGIBuffer`；Producer mutex lease 由 tracked-sample 回调持有至最终引用释放。后台 worker 与最终回调均属于 DLL 活跃对象，Shutdown 必须取消同步 pipe I/O 并确认 worker 退出，不能在代码仍可能执行时允许卸载。绑定 D3D manager 后只交付同 device 的 native NV12 实时画面，pipe 忙、逐帧失败或代际耗尽都不切到 CPU；没有 manager 的客户端才走独立 Shared Frame Ring CpuBridge。隐私占位不是 native 故障降级：PICQ ring 头部以不产生 pixel demand 的 checked `Live/Placeholder` content signal 表示控制状态，槽位再携带对应像素类型；GpuNative 的 Live 状态不激活 ring pixel reader。最终准入 token 同时绑定 ring mapping epoch 与 content signal，不能只比较会在 Receiver 重启后重用的 mapping 内 revision。进入 Placeholder 时 Producer 清除 retained native frame、断开当前 control channel，Consumer 在最终 QueueEvent 准入点复核相同 live token，推进 native session 并只为该显式占位生成自有 CPU sample；初始无 Producer 直接为 Placeholder，短断连 hold 只适用于已经观察到的同一 mapping 实时画面；信号前已经通过最终复核的 sample 视为 in-flight，不承诺从外部进程撤回，信号后的旧 prepared sample 不得入队。恢复 Live 时再次重建 native 会话。断连 hold 到期同样必须进入 Placeholder，不能无限保留最后一张 native 实时画面。
...
Windows CPU IPC 内容失效由独立原子代际表达，与文件/映射的进程世代分开。每次准备携带内容代际，槽位提交保留该原始值；Consumer 获得读 lease 后再次检查槽位值与当前值相等，不能把复制后的最新代际贴到旧内容上。Owner 只推进标量，不等待复制或 GPU 任务，也不覆盖消费者仍持有的像素。此门禁使迟到旧发布不可重新获得 lease；已交付系统 sample、缓存与持有引用的隐私期限仍由各 sink 的清理合同负责。代际耗尽永久关闭该映射，不能归零后重新启用。macOS CpuBridge 不建立自定义 CPU IPC，仍通过与 GpuNative 相同的 CMIO sink 队列交付独立 IOSurface-backed 图像。

Windows CPU demand 必须来自实际 ring 消费请求，不能把已打开映射或已安装 VCam 当作持续需求。消费租期使用同一主机的单调时钟，最长 250ms；有请求才续期，消费者崩溃后不永久保留需求。输出 owner 只保留一个最新待处理原生源，等待 demand 时不准备 GPU 目标或 readback；当前内容代际内已成功导出的同一源身份不重复物化。macOS 由 Camera Extension 聚合 source 客户端生命周期：最后一个 source 客户端停止后 sink 不再继续消费，Host 队列至多保留固定三张，不形成持续 readback。需求只控制 VCam 输出工作，不调整源 codec、帧率、Decoder 或其他 sink。

macOS VCam 的 GpuNative renderer 与 CpuBridge 各自在所选固定布局使用三槽池；同一工作者只实例化当前选择的路径。CMIO output queue 也固定三槽，系统未移除 token 前 Host 不释放图像 owner。Extension 只为无 Host 图像时的黑帧使用每分辨率共享的三槽池；live 图像直接缓存系统消费的 sink PixelBuffer，不另建缩放池。CoreVideo 拒绝扩池时跳过该次准备，不能覆盖已交付图像；恢复由外部 sample 引用释放决定。此边界不替代全链路总预算或系统已持有样本的隐私期限。
...
Android设置与直播选择使用完整VideoSourceFormat；持久化默认请求与Core最后提交事实分开。准备查询由ViewModel作用域持有，在后台按镜头/方向更新，Compose只渲染不可变候选与触发回调。新连接先等待摄像头权限、本地准备及认证后的完整Decoder能力，再创建明确源事务；此前的本地预览AU不能自行绑定该连接。默认请求不可用时按当前镜头与Receiver的完整共同上限准备初始视频。原生失败不由界面轮询无限重启；恢复仍由Core事务驱动。界面复用既有PicooSheet和官方Compose组件，无新增UI或媒体依赖。

手机方向变化可能要求更大的Camera2输入，不能只更新GPU旋转后从不足的像素放大。CaptureProfile包含所请求的显示相对方向，原生输入选择、GPU准备与失败恢复采用同一份profile。方向变化保持既定SourceFormat，通过明确的原生世代刷新提交；不可准备时拒绝该方向意图并保留已提交事实，同一次失败意图不由轮询自动重试。

iOS的VideoSourceFormat贯穿相机请求、VideoToolbox配置和Core事务；默认意图为AVC1080p60，最后已提交事实独立于请求。候选由同一AVFoundation format的尺寸/fps与实际要求硬件的VideoToolbox准备会话求交，临时会话顺序创建并立即释放；复用生产属性配置，不引入第二套编码器规则或依赖。准备不等于首AU实际记录已准入，也不代表持续帧率。SwiftUI用既有Picker/Menu表达完整候选，不组合独立最大值；不可用默认按完整共同上限适配，无共同候选时保留连接并报告媒体未就绪。恢复缓存保存完整codec/尺寸/fps，AU必须匹配完整格式和原生世代才能完成事务。

用户明确切换镜头时，Sender按目标镜头的完整准备上限自动适配：有当前codec候选则保留codec，再按分辨率、fps降序选择同一条候选；若当前codec没有候选才使用其他可准备codec。直播时上限取目标镜头与Receiver的交集，并在同一原生profile和Core事务中切换；未连接时上限只取本地可准备候选，只重建本机取景器，不分配stream_epoch。成功后呈现实际使用的格式，失败恢复旧镜头和完整格式。此自动适配属于镜头切换意图，不扩展为画质菜单明确选择或运行中热降级。异步查询期间会话、方向或源事实改变时废弃结果；无交集不破坏正在使用的镜头。

安全连接/配对与原生媒体准备是独立边界。本地相机候选未知或默认格式不可用不能阻挡连接，也不能因准备失败断开已授权连接。连接后若默认请求不在当前镜头与Receiver交集内，按同一完整上限策略准备初始视频；没有共同候选时保持连接并报告媒体未就绪。运行中的明确格式选择仍不降级。

桌面预览需求由GPUI窗口显示回调发出，每次需求至多消费一次；源空档不会使需求过期，也不依赖视频完成才能触发下一次显示。窗口遮挡与恢复复用GPUI原生显示调度，不以最近一次绘制的时间猜测可见性；没有新显示需求不重复提交GPU预览，其他输出不受预览调度影响。复用既有GPUI request_animation_frame，无新增计时或可见性探测依赖。

iOS原生传感器输入尺寸和呈现旋转分别描述。RotationCoordinator只提供方向意图与本地预览方向，不能原地修改已经绑定VideoToolbox回调的编码配置。直播方向意图经Core事务重建原生编码世代，与codec/尺寸/fps及方向一起等待匹配的首AU后提交；失败恢复保留先前方向。同一失败方向不会随轮询重复执行，新的方向意图或新连接才可重新尝试。此路径保留原生传感器画幅，不以不足像素的裁剪放大模拟固定横向画布。

iOS已经通过同一AVFoundation格式条目准备精确传感器输入，编码提交直接持有该原生图像。输入像素尺寸不符合该准备配置时报告原生失败，不启用隐式像素传输缩放或裁剪，也不通过输入宽高关系猜另一种编码尺寸。呈现旋转独立于传感器尺寸，输入事实错误由事务失败/恢复处理。明确的输出几何处理仍属于相应GPU输出边界。

原生录像段适配位于picoo-recording，独立于Decoder、FrameBus和UI。macOS段通过AVAssetWriter直通标准压缩AU，固定该段sample description，验证段首闭合IDR、参数集身份及递增源PTS；暂时不可写返回Busy且不消费AU，重试时限由专用录像工作者负责。段适配只最终化partial文件，录制owner在确认成功和必要文件同步后负责完成文件命名与manifest更新；原生失败不得推进完成状态。CoreMedia样本的NotSync标记来自共享位流分类，媒体timescale显式采用微秒以保持源PTS。

录制bundle独占创建私有目录，不重新打开旧状态。只有原生段最终化凭据才能提交完成段；段同步、无覆盖提升文件名后，manifest通过同目录临时文件原子替换。磁盘错误或容量耗尽使结果进入不可恢复的Failed，后续finish不能覆盖；gap使最终结果为HasGaps。未登记文件不能推断为成功，partial保留供诊断。文件操作和流式摘要只在录制工作者执行；Unix同步目录，其他平台的断电耐久性须独立验证。

原码流录制工作者独占原生段、短时重排和bundle。Receiver仅提交完整AU、读取原子状态/一次性结果及消费合并的关键帧请求；停止通过关闭生产者排空，不能在Receiver或UI等待线程join。全进程最多一个工作者；旧owner完成全部原生工作与清理后先释放名额，再发布最终结果，结果可见即表示下一owner可安全创建。系统I/O迟滞不能通过不断创建新线程绕过上限。等待可写IDR设独立期限，编码/磁盘错误只终止录像。

录像的完整AU入口位于重组完成后、live配置等待与恢复选择前。可靠配置晚到时，由独立有界门禁保存AU及连接/epoch身份；已知配置快照不能被后续配置覆盖。匹配配置到达后交给录制重排，未知配置不能使用旧sample description。live只保留单个恢复候选的策略不适用于录像；等待超限或停止时仍缺配置必须使录像失败。

Apple段创建时保留原生输出的文件句柄，原生取消完成后恢复被删除的partial路径。恢复只在录制工作者执行，使用无覆盖文件提升；外来路径替换或存储失败必须报告。取消后的字节留存不等于容器完整或可播放，也不能产生最终化凭据。正常完成不复制段内容。

桌面录像入口只出现在连接成功后的 Live 顶栏，与分辨率等操作并列，按钮文案为「录像」，菜单内分别开始/停止原码流与处理后录像；待机连接页与工作区标题行不显示录像入口。切换页面不改变录制生命周期。保存位置使用平台原生文件夹选择器，取消不创建录像；开始/停止走Receiver命令队列，接纳回复不代表文件已保存。状态来自录制owner快照，区分准备、录制、收尾和最终的完整/缺口/失败结果；保留目录可由系统打开。UI只持有尚未回复的命令与操作错误，不执行媒体或磁盘工作。

桌面将原码流与处理后录像呈现为两个独立消费者，分别开始、停止、观察结果并保留各自目录；一个模式的活跃状态不代替或禁止另一个模式。处理后录像开始时显式选择AVC/HEVC与不高于当前源的30/60fps，输出尺寸固定为开始时的当前720p/1080p源规格，不随预览窗口、页面或后续UI布局变化。当前无效果编辑器时scene revision固定为身份场景零；不能借用传输配置revision虚构效果变更。macOS与Windows在编入各自原生处理后owner时显示该入口；其他平台保持不可用，不能用原码流成功伪装能力可用。

录制工作者的推进时间由接收侧独立观察，不能由同一被阻塞的原生调用负责宣布超时。超过观察期限时呈现写入无响应；该状态只说明线程未推进，不等于文件失败或原生对象可释放。最终文件结果仍只由录制owner发布；owner的原生工作与清理未返回前保留资源名额，完成清理后释放名额并发布结果，UI不强杀线程或接管文件。已终止入口不再请求关键帧。

录像压缩输入的字节预算覆盖配置等待、入口通道、短时重排及正在提交的AU，不为每个阶段各授一份额度。录制入口预留AU载荷及最大配置记录，所有权随输入移动，写入返回或输入丢弃才释放。进程级初始上限16MiB；超额报告录像失败，不反压直播。该额度不声称包含平台mux内部池、网络重组或native图像预算。

完整AU首次进入录制边界时绑定本地单调时钟的绝对提交期限。配置等待、入口通道、重排和停止排空只传递该期限，不能在下一个阶段重新起算。原生调用前后均验证剩余期限；超期使该录像失败并保留已接受的有效前缀。此准入期限不承诺能中断正在执行的系统I/O，也不限制用户正常结束时单独的最终化工作。

Apple处理后录像的编码适配位于picoo-recording，只消费已完成的picoo-gpu目标图像。创建时要求硬件AVC/HEVC并核实实际硬件属性，显式设置profile、零重排与色彩；不允许AVAssetWriter自行选择编码器。编码回调绑定提交时的源身份和PTS，保留目标图像到完成，不将调用返回或等待超期视为硬件读取结束。适配器有界接纳与输出，超期/丢帧/原生错误终止本实例；输出是压缩AU与实际配置，汇合到独立mux边界。FrameBus订阅、媒体时间采样和场景分段仍由RenderedRecorder owner拥有。

Windows处理后录像硬编同样位于picoo-recording，只从`MFT_ENUM_FLAG_HARDWARE`枚举AVC/HEVC encoder MFT，不把系统软件encoder或Sink Writer的“允许硬件transform”提示当成硬件证据。硬件MFT按官方异步事件模型驱动；必须声明D3D11 aware，绑定由目标NV12纹理所属的同一D3D11 device创建的DXGI manager，再按output先于input的顺序提交固定profile、level、码率、尺寸、30/60fps与BT.709 limited类型。NeedInput是可提前到达的输入额度而非与HaveOutput严格交替的响应，但owner仍只在取得前一输出后消费下一额度。输入sample直接包装已完成的D3D11目标surface，不做CPU readback；拒绝声明会跨对应ProcessOutput继续持有输入buffer的MFT，一个在途输入持有RenderedImage直到对应输出取得；AVC在类型协商前强制零B帧，HEVC不调用其未支持的该属性，两者都用低延迟模式要求一入一出且PTS精确保留。每帧IDR请求通过ICodecAPI显式提交，输出AU、参数集、profile、尺寸、色彩和随机访问事实重新验证后才进入WindowsSegment。事件或输出等待有界，调用方分配的压缩输出buffer遵守MFT声明的大小与对齐；超期、格式变化、丢帧、device/MFT错误使实例失效，不能切到软件或不同device继续。

Windows与macOS处理后录像共用RenderedRecorder及其FrameBus订阅、媒体时间采样、gap与代际分段、bundle和工作者生命周期；Receiver和桌面也共用双模式命令与状态契约。平台分流只存在于renderer、硬件encoder和原生mux适配边界。Windows renderer先从首帧源图像取得D3D11设备并生成NV12目标，encoder再从首个目标取得同一设备；两者都由同一处理后录像工作者拥有，不能把平台初始化搬回Receiver/UI，也不能复制一套Windows业务状态机。

Windows原码流段使用MF压缩直通，输入按平台要求转Annex B并提供实际参数集序列头，由已验证的系统sink维护sample description。AVC使用fragmented MP4，HEVC使用普通MP4并保留独立RAP分段；不运行中改变容器或codec来隐藏错误。每个提交由SinkWriter marker确认处理后才释放该输入预算，最多一个待确认样本；最终化回调成功才产生完成凭据。COM/MF启动、原生对象创建和失效清理属于同一录制线程，超期不意味着系统调用可强制中断。

Windows与macOS原码流录像共用EncodedWriter、录制工作者、Receiver完整AU入口和桌面状态契约，只在原生段适配处选择平台实现。Windows输入校验失败不污染已接受的有效前缀；原生写入开始后若未确认完成，则作废该段，不能将不确定的原生结果宣称为完成。
