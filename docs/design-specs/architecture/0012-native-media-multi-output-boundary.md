...
用户明确切换镜头时，Sender按目标镜头与Receiver的完整准备交集自动选择上限：有当前codec候选则保留codec，再按分辨率、fps降序选择同一条候选；若当前codec没有候选才使用其他可准备codec。镜头与源格式在同一原生profile和Core事务中切换，成功后呈现实际提交格式，失败恢复旧镜头和完整格式。此自动适配属于镜头切换意图，不扩展为画质菜单明确选择或运行中热降级。异步查询期间会话、方向或源事实改变时废弃结果；无交集不破坏正在使用的镜头。
