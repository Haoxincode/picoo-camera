...
- **设置页**：显示名称、自动接受已配对设备、开机启动、占位画面、日志级别、已配对设备、虚拟摄像头修复、诊断导出；最小化到系统托盘仅在 Windows 显示，macOS 关闭窗口后保留在 Dock/后台。

桌面一级导航由 GPUI View 持有进程内展开/折叠状态：展开态遵循 HTML 原型的 `204px` 导航布局，折叠态收敛为 `48px` 图标栏。该状态只改变视图几何与标签可见性，不进入 `ReceiverRuntime`、协议状态或跨设备偏好；导航按钮在两种状态下保持相同的稳定 ID、页面 Action、选中态和无障碍语义。
