...
- Receiver 启动时必须区分身份/可信存储损坏与普通传输、解码或监听失败。前两者进入独立恢复界面，
  且在用户明确提交前不得创建 QUIC/媒体 Runtime；身份修复覆盖 OS 安全存储中的损坏凭据并原子清空
  可信存储，可信存储修复只原子写入空的有效存储而保留 Receiver 身份。正常运行时“重置全部配对”是
  用户可直接提交的破坏性操作，提交期间禁止重复命令并在成功后立即断开当前 Sender、反馈结果。任何恢复结果都不得从旧密钥或
  损坏记录推导信任，双方必须重新核对短码。
