...
| Quinn + `socket2` 预构造 UDP socket | 采用。`socket2 0.6` 是 Quinn 自身使用的 MIT/Apache-2.0 成熟 socket 封装；只用于在 Quinn 接管前保留双栈行为并应用平台官方的单 socket 网络绑定，不自行实现 UDP/QUIC。`all` feature 仅开启已封装的平台接口绑定 API。 |
...
`android_setsocknetwork` 或 `socket2` 的 `bind_device_by_index_v4/v6`（其 Darwin 实现对接
`IP_BOUND_IF` / `IPV6_BOUND_IF`），最后把 socket 交给 Quinn。Android Network 句柄仍然不是网卡
索引，不能机械替换为 `socket2` 的接口绑定。
