...
移动端采用相机主页面优先的壳层：Sender 首屏始终保留取景与相机控制空间，连接状态胶囊承担发现与连接入口；
发现、手动 IP、配对和等待使用覆盖在 Camera context 之上的原生 Sheet/Overlay，Settings 才是独立页面。
这些临时面板仍消费 Control context 的表面和组件语义，不把在线设备列表作为一级导航，也不把已信任身份记录
从本地安全存储中移除。
