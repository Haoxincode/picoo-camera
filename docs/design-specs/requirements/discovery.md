...
| REQ-PICOO-DISCOVERY-008 | planned | PUC-002 / ARCH-PICOO-DISCOVERY-001 | iOS 在用户发起连接流程时请求 Local Network 权限并发现 Receiver | `NSLocalNetworkUsageDescription` + `_picoocam._udp` Bonjour 声明；C ABI discovery browser；拒绝权限时保留手动 `IP:端口` 直连 |
