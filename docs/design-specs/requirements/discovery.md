...
| REQ-PICOO-DISCOVERY-005 | implemented | PUC-002 | Android 使用系统 NSD 浏览 Receiver，但 TXT 协议字段由 Rust Core 统一校验 | `NsdReceiverBrowser` 将原始 TXT 交给 JNI `parseDiscoveryTxt`；Rust `ReceiverAdvertisement::from_txt_properties` 强制完整字段、白名单与 pairing_state；Devices 行展示 Ready/Paired |
