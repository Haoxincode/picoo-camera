...
| REQ-PICOO-DISCOVERY-004 | implemented | PUC-003 | 短期 nonce 过期后二维码不可复用 | `qr_nonce_expires` 单元测试 |
| REQ-PICOO-DISCOVERY-005 | implemented | PUC-002 | Android NSD 浏览 Receiver 列表 | `NsdReceiverBrowser` + TXT 白名单解析测试 |
| REQ-PICOO-DISCOVERY-006 | implemented | PUC-002 / PRD §8.1 | 健康网络发现 P50&lt;2s；已配对连接建立 &lt;3s | `paired_connect_to_streaming_under_three_seconds`；mDNS `browser_discovers_local_advertiser_under_two_seconds`（`--ignored`，需真实 mDNS） |
