...
| REQ-PICOO-DISCOVERY-003 | implemented | PUC-003 | 桌面 Show QR Code，含地址/端口/ID/版本/指纹/nonce | QR 使用 LAN advertise host（非 `0.0.0.0`）；`qr_json_payload_connects_to_streaming`；Android 扫码用 CameraX 1.4.2+（16KB native）+ ML Kit，预览/编码仍走 Camera2；CameraX 仅在 `AndroidView.factory` 绑定，扫描线重组不得 `unbindAll`；`QrScanSessionTest` |
