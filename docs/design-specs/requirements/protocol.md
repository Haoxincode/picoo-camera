...
| REQ-PICOO-PROTOCOL-009 | implemented | ARCH-PICOO-SESSION-001 | `ReceiverStats.packet_loss` 使用同一统计单位：不完整 AU 中确认缺失的 fragment /（收到 fragment + 确认缺失 fragment）；不得混用 AU drop 与 Datagram 数，且明确其不能观察完全无片到达的 AU | Reassembly missing-fragment 计数；ReceiverStats 同单位比率测试；后续独立 packet sequence 另行评审 |
