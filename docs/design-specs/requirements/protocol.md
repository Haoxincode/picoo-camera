...
| REQ-PICOO-PROTOCOL-010 | implemented | ARCH-PICOO-PROTOCOL-001 / ARCH-PICOO-SESSION-001 / ARCH-PICOO-RUNTIME-001 | AU 数据片按平衡的最多 6 片一组；Sender 对健康 delta/轻微损伤/明显损伤分别发送 0/1/2 个 Reed-Solomon parity，IDR 固定强保护；同一 shard position 跨组轮转且所有数据片先于 parity；Receiver 在 PartialFrame 创建时建立 fragment→group 映射与组内位图，每片只检查受影响分组，并在 deadline 前按实际可用 parity 恢复最多 2 个真实缺片；Sender 只计算实际发送的 parity 并复用 shard scratch；超过能力仍丢整帧并恢复参考链 | `ranges_avoid_single_fragment_tail`；FEC 单片/两片恢复、迟到原片、无 parity 近似线性 group-check 与超预算测试；`adaptive_fec_emits_zero_one_or_two_parity_shards`；Light parity 0 与 Strong parity 0 逐字节一致；`fec_schedule_spreads_groups_and_keeps_parity_after_source_data`；`reassembly_fec` benchmark 覆盖 12/64/256 片、64B/`MAX_FEC_FRAGMENT_PAYLOAD` 近 MTU payload、冷启动/连续复用、乱序、每组 1/2 片丢失，并断言实际完成 AU 数且报告 allocation/group-check/recovery；真机弱网统计仍待 |
