...
| REQ-PICOO-TRANSPORT-005 | implemented | PRD §22 验证 A | Android/Windows quiche 可构建链接 | Android：`cargo ndk`→`libpicoo_ffi.so` + Gradle APK/AAB（含 16KB 门禁）；Windows：`xtask build/package windows` 产出 exe/DLL/MSI（CI `windows-latest`） |
