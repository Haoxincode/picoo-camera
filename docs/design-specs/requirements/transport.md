...
| REQ-PICOO-TRANSPORT-005 | implemented | PRD §22 验证 A | Android/Windows quiche 可构建链接 | 本地 `cargo ndk` 产出 `libpicoo_ffi.so` + `assembleDebug` APK；Windows/CI job 仍待 Actions 配额 |
