...
| REQ-PICOO-UI-005 | implemented | PUC-005 | Sender 前台、防锁屏、过热/低电量提示 | FGS + `FLAG_KEEP_SCREEN_ON` + `PowerHints`（真机验证仍待） |
...
| REQ-PICOO-UI-008 | implemented | PRD §16 | 关闭窗口时托盘策略 | `tray.rs` CloseAction + Settings 写入 `tray_policy`；Win32 `Shell_NotifyIcon` 待补 |
