...
| REQ-PICOO-STACK-003 | implemented | ARCH-PICOO-STACK-001 | Android 通过 Rust JNI exports、iOS 通过 XCFramework Clang module 的 C ABI 调用 Rust Core | Android `cargo-ndk` 单一 Rust JNI `.so` 须 16KB `PT_LOAD` 对齐；iOS `PicooCore.xcframework` 携带 header/module map，`PicooSenderSessionTests` 覆盖 Swift 创建、查询、销毁 Sender handle；远端 Simulator 绿测后升级为 `verified`；`scripts/check_android_so_16k.sh` |
...
| REQ-PICOO-STACK-007 | implemented | ARCH-PICOO-STACK-001 / ci-and-build.md | Apple 平台具有独立、无签名的 ARM64 可编译基线 | 远端已验证 ARM64 GPUI Receiver 与 device/simulator XCFramework；`xtask build ios` 本地通过 Clang smoke 与 SwiftUI App 完整链接；App zip artifact 与 Simulator XCTest workflow 已配置，远端验证待完成 |
