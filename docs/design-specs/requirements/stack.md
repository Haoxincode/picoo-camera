...
| REQ-PICOO-STACK-003 | planned | ARCH-PICOO-STACK-001 | Android 通过 Rust JNI exports、iOS 通过 C ABI 调用 Rust Core | Android `cargo-ndk` 单一 Rust JNI `.so` 须 16KB `PT_LOAD` 对齐；iOS `PicooCore.xcframework` 携带 `picoo_camera.h` 与 module map，Swift 调用验收仍待；`scripts/check_android_so_16k.sh` |
...
| REQ-PICOO-STACK-007 | implemented | ARCH-PICOO-STACK-001 / ci-and-build.md | Apple 平台具有独立、无签名的 ARM64 可编译基线 | `xtask build macos` 构建 ARM64 共享 GPUI Receiver；`xtask build ios` 产出含 device/simulator arm64 slices 的 `PicooCore.xcframework`，并由 iPhone Simulator SDK Clang 完整链接 C ABI smoke；CI `macos` / `ios` job 上传对应 artifact |
