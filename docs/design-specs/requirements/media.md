...
| REQ-PICOO-MEDIA-011 | planned | ARCH-PICOO-MEDIA-001 | iOS 使用 AVFoundation 采集并由 VideoToolbox 低延迟硬件编码 H.264；原始帧不跨 Rust FFI | 已实现按需 Camera 权限、actor 串行化 `AVCaptureSession`、1080p/720p preset、预览和前后摄切换；待实现 `AVCaptureVideoDataOutput` → VideoToolbox、无 B 帧 Access Unit → `PicooCore`、`stream_epoch` 与 iPhone 真机 720p30 / 1080p30 验证 |
