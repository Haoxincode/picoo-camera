...
| REQ-PICOO-MEDIA-010 | implemented | PUC-006 | 持续拥塞降至 720p；健康后可回升 1080p；过热强制 720p | rate-control + Android；`abr_downshift`/`abr_upshift` FrameHub；`thermal_hold` 阻止上调；`sync_encode_height` + JNI；`PowerHints.shouldForce720p` |
