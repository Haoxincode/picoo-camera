...
| REQ-PICOO-MEDIA-060 | implemented | ARCH-PICOO-MEDIA-002 / REQ-PICOO-NEXT-005、006、026 | 手机与电脑触发Android镜头切换共用目标镜头完整准备；直播时按用户明确授权自动选择目标镜头与Receiver交集上限，未连接时按本地可准备上限切换取景器且不创建Core epoch；优先当前codec、其次分辨率、再fps；直播路径镜头和格式一次提交，异步准备后核对原会话/方向/源事实，无交集保留原镜头；画质菜单的明确格式请求仍不降级 | 不支持目标保持原镜头，前后摄可准备格式实际切换与fps合同；未连接取景器翻转不依赖已提交源格式；完整GUI链路验证 |

| REQ-PICOO-MEDIA-061 | implemented | ARCH-PICOO-MEDIA-002 / REQ-PICOO-NEXT-005、006、026 | iOS镜头切换按与Android一致的codec、分辨率、fps优先序选择目标完整上限；直播时与Receiver求交并走Core事务，未连接时按本地可准备上限切换取景器；镜头与完整格式在同一次原生配置中提交，目标码率由实际目标高度派生，失败恢复旧事实 | Swift上限选择与回退合同、未连接本地候选合同、iOS构建；iPhone真机切换及持续fps另验 |
