...
| REQ-PICOO-GPU-006 | implemented | ARCH-PICOO-MEDIA-002 / REQ-PICOO-NEXT-009、016、029、031 | Windows 原生 Video Processor 按 RenderSpec 输出 NV12 BT.709 limited；检查驱动颜色/旋转/镜像与无历史帧依赖能力；crop 按 rotation→mirror→clip 变换；输出池三槽且完成事件保留源/目标 | 八种几何组合、1088 补齐区裁剪、真实 D3D11 输出池保留与扩池拒绝；Windows 原生 CI、真实显卡像素对照；主链路与 Exporter/Preview 接入另验 |
