...
| REQ-PICOO-MEDIA-052 | implemented | ARCH-PICOO-MEDIA-002 / REQ-PICOO-NEXT-003、004、009、025 | 原生 Decoder owner 内逐项 probe，只有实际原生输出与原始 token 成立才生成完整 offer；探测图像不外泄，每项 reset，无软件替代，reset/身份失败中止整份结果 | M4 实际11条完整offer、24项Decoder回归、全拒绝不继承样本能力与reset失败停止；Receiver能力发送接线及持续吞吐另验 |
