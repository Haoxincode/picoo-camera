...
| REQ-PICOO-FRAME-009 | planned | ARCH-PICOO-RUNTIME-001 | 同进程 FrameHub 改为 LatestFrameStore，使用 `Arc<VideoFrame>` 的容量一/latest-only 语义；Shared Frame Ring 名称和并发协议保持不变 | 慢 Preview/VCam 不反压 Decoder；名称/API/Architecture 一致 |
| REQ-PICOO-FRAME-010 | planned | ARCH-PICOO-RUNTIME-001 | Decoder、Preview、Shared Ring Writer 与可选 Recorder 共享不可变帧，并由有界 FrameBufferPool 管理可复用像素内存 | 每帧主要分配一次；消费者只增减 Arc；长期 soak 无增长 |
