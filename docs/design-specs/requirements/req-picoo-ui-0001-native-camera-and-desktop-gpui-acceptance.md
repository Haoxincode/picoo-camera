...
- [ ] **AC-M-SHELL-01（主题与表面）**：移动端默认进入单一 Camera context 主页面；连接状态、Manual Connect、Pairing 与 Waiting 作为该页面上的状态层或原生 Sheet，Settings 保留为唯一独立页面。连接与设置面板使用 Control context 并跟随系统明暗；不得继续以应用级强制 Dark mode 覆盖系统偏好。
...
### 3.1 相机主页面与连接入口（单一主页面）
- [ ] **AC-M-DISC-01**：移动端首屏直接进入全屏本机相机取景。顶部居中显示可点击的连接状态胶囊，右侧保留语义 `settings` Icon button；视觉 glyph 使用 standard 尺寸，点击目标遵守平台最小触控尺寸。已连接时胶囊显示设备名与链路指标；未连接时显示「点击连接电脑」或「正在寻找电脑」。
- [ ] **AC-M-DISC-02**：连接状态胶囊是发现与连接的唯一主入口，不再把在线设备列表作为一级页面。点击后升起原生 Connection Sheet；主流程只描述“搜索同一 Wi‑Fi 下的电脑”等用户任务，不展示 Bonjour、NSD、mDNS 或 `_picoocam._udp` 等实现标识。Reduced Motion 下停止发现动效但保留文案与颜色变化。
- [ ] **AC-M-DISC-03**：Connection Sheet 默认只显示当前即时发现的 Receiver；单台设备可直接点按连接，多台设备才显示可扫描的 `DeviceRow`。已配对设备使用信息/选中语义，设备身份管理不在主连接面板展开，统一进入 Settings 的已配对信任电脑。
- [ ] **AC-M-DISC-04**：未连接状态仍保留相机主页面与底部控制区；连接动作使用 Camera context 的连接图标和明确文案，避免把“空设备列表”作为主视觉。连接失败、权限缺失和网络不稳定通过预览上的状态层表达。
- [ ] **AC-M-DISC-05**：Connection Sheet 内的发现面板区分「等待发现权限」「正在搜索」与「未发现」状态，并始终保留清晰的「通过 IP 地址连接」次级动作。发现到 Receiver 后从该 Sheet 进入双端短码核对 Sheet，不导航到新的设备页。

### 3.2 手动 IP 直连抽屉（Connection Sheet 内的 Manual Connect Sheet）
- [ ] **AC-M-MANUAL-01**：从主页面连接状态胶囊或 Connection Sheet 升起原生 Sheet，只接收局域网 IPv4 与端口（如 `192.168.1.108:4433`）；地址使用四段数字输入格，点号和冒号由界面固定呈现，端口默认预填 `4433` 且可编辑。每段达到可确定边界后自动前进，键盘 Next 可前进较短段；粘贴完整 `IP:端口` 时自动分配到各输入格。通过 mDNS 选择设备时不展示此输入。紧凑窗口可使用半屏，键盘或较大文字需要更多空间时允许扩展到大 Detent/完整页面。原生 Compose/SwiftUI 输入与焦点 API 已满足该单一用途，不引入第三方 OTP、地址或表单组件依赖。
- [ ] **AC-M-MANUAL-02**：输入 IP:端口后，Sender 直接向目标 Endpoint 发起 QUIC/TLS 握手；连接建立后进入与自动发现路径相同的双端短码核对，短码不得被解释为地址查询码。
- [ ] **AC-M-MANUAL-03**：连接流程本身（Connection Sheet、IP 直连、短码核对）不请求相机权限。主页面本机取景器在前台按需申请权限；未授权时仍保留连接入口与翻转控件，授权后即可在未连接状态下预览和切换前后摄。
- [ ] **AC-M-MANUAL-04**：连接流程不提供二维码生成或扫码入口，不引入 ZXing、ML Kit Barcode Scanning 或其他扫码 SDK。

### 3.3 配对确认与等待态（主页面上的 Pairing/Waiting Sheet）
...
- [ ] **AC-M-LIVE-01（顶部 HUD）**：
  - 顶部居中两行显示可点击的连接状态与实时遥测。第一行为品牌蓝连接点 + `电脑设备名 已连接`；未连接时显示 `点击连接电脑` / `正在寻找电脑`，点击打开 Connection Sheet；第二行为 RTT、实时码率、分辨率和帧率（如 `63ms · 1.9 Mbps · 720p / 30fps`）；
  - 遥测数字使用等宽表格数字；分辨率段保持可点击并直接一键秒级轮换 `1080P / 720P / 480P`，不弹出抽屉或菜单；
  - 亮度、镜像与防误触控制移至底部第一排等宽控制区，依次使用语义 `exposure`、`mirror`、`interactionLock` Reicon；激活时使用 Camera context selected token，并通过可访问状态描述报告当前值。
- [ ] **AC-M-LIVE-02（设备过热）**：
  - 设备过热时画质药丸使用 Warning token，并附带语义 `overheat` Reicon 与 `720P · 30` 文案；
  - 强制降档至 720P，点击弹出 Toast 提示 `设备偏热保护中，1080P 暂不可选`。
- [ ] **AC-M-LIVE-03（全屏取景器手势）**：
  - 电脑端输出固定为横向 `16:9`，不提供 `9:16` 输出切换。Sender 本机取景器使用全屏 center-cover，不因手机/相机比例不同出现黑边，也不叠加四角参考框；
  - 手机竖持时本机取景器不声称逐像素等同电脑端；电脑端继续取直立相机画面的中央横向 16:9，手机横持可更直观地观察最终构图；
  - 点击取景器任意坐标，生成黄色方框对焦环（Focus Ring），伴随向内微缩与 800ms 渐隐淡出动效；
  - 双击取景器空白区域进入全屏沉浸模式（淡出所有浮层）。
- [ ] **AC-M-LIVE-04（极简调光条）**：
  - 底部 `亮度` 控件单击直接循环档位，不展开二级面板；档位严格映射为 `自动` / `提亮 +1` / `提亮 +2` / `压暗 -1` / `压暗 -2`；
  - 视觉 Icon 使用 emphasis 尺寸，按钮使用平台最小触控目标；非自动档位的 Icon 与状态文字使用 selected token；
  - 取景画面实时叠加明暗滤镜（`brightness`）反馈。
- [ ] **AC-M-LIVE-05（底栏监控与镜头文字移除）**：
  - 码率、帧率与 RTT 统一进入顶部第二行遥测，不在底部重复显示监控药丸；
  - **彻底移除“前置镜头/后置镜头”等多余文字注释**（用户看取景画面即可直观确认）。
- [ ] **AC-M-LIVE-06（停止录像大圆钮）**：
  - 底部第二排左侧使用语义 `disconnect` Reicon 和 Danger token，右侧使用语义 `switchCamera` Reicon；两项等宽并以分隔线建立关系，不使用悬浮卡片；
  - 轻触断开一次进入 Armed 状态并提示 `再次点击确认断开`，3 秒内二次点击安全挂断退出，3 秒超时自动复位。
- [ ] **AC-M-LIVE-07（原生翻转镜头按钮）**：
  - 右下角使用 Camera context 圆形控制，内嵌语义 `switchCamera` Reicon；视觉 Icon 与平台触控目标分离；
  - 未连接时翻转只切换本机取景器前后摄；已连接推流时点击后图标自身带有 180° 平滑旋转动画，取景器伴随瞬间重聚模糊滤镜，底层递增 `stream_epoch` 并请求 IDR 关键帧。
- [ ] **AC-M-LIVE-08（网络中断重连遮罩）**：
  - 网络突发中断时严禁直接闪退回主页，必须弹出半透明模糊重连遮罩；
  - 显示退避阶梯（`500ms` ➔ `1s` ➔ `2s` ➔ `5s`）并提供 `停止重连并退出` 按钮。

### 3.5 设置中心（Screen 6）
- [ ] **AC-M-SET-01**：顶栏左侧使用 `Chevron Left` 矢量返回图标，返回唯一的相机主页面。
...
- [ ] **AC-D-LAYOUT-01（主区域优先的稳定分栏）**：
  - 左侧承载主机标识、配对短码状态、开始使用指引、实时预览及主卡内部底部的横向网络状态区，使用 `flex_1` 消费窗口的全部剩余宽度；网络状态不得另建带圆角、背景、阴影或外边框的独立长卡；
  - 待机态右侧只承载「设备与连接状态机」并填满连接工作区的可用高度，允许在 21–24rem 内收缩且不消费窗口新增宽度；进入 Live 后移除整个右侧设备栏，全部工作区归实时预览和单行顶栏；
...
### 4.3 右侧设备连接状态机与左侧网络状态栏
- [ ] **AC-D-DEVICE-01（已信任与最近连接设备列表）**：
  - Receiver 不主动发现或连接 Sender。待机时设备卡只呈现本机可信设备存储中的已信任/最近连接设备；每条设备固定为名称、人类可读的最近连接时间（`今天` / `昨天` / `N 天前`）、可区分身份指纹前缀三行，等待/离线由尾部状态承担；24rem 舒适宽度与 1440×900 默认窗口下不得截断时间或指纹，不提供伪造的 `[ 连接 ]` 动作；
  - 同名条目必须显示至少 8 位且发生碰撞时自动延长的公钥指纹前缀，并按最近连接时间稳定排序；各删除按钮的 tooltip 和无障碍名称必须同时标明设备名与身份前缀，点击后立即异步撤销该设备信任，提交期间禁用重复命令，成功从列表移除并给出成功反馈，失败时在当前连接页显示错误；未知新身份完成配对后如存在同名历史身份，使用不可点外部关闭的 AlertDialog 逐条列出候选指纹并明确提供“仅保留当前身份 / 保留全部”；配对 Dialog 从发出关闭请求到 `on_close` 完成期间仍由其生命周期占有窗口，清理提示必须等待它实际关闭后再打开，不得被迟到的关闭任务误关或抢焦点，决策必须跨重启恢复且可信重连不得重新生成；替换只执行所展示的 revision 快照，候选失效或持久化失败时保留 Dialog 并显示错误；
  - 卡片标题使用固定版本 Reicon 手机语义图标；设备条目左侧使用 Picoo 自有的中性手机机框 SVG，并以机框高度建立稳定三行行高。机框可呈现屏幕、边框、听筒与镜头层次，但不得把 Android 等非 iPhone Sender 冒充成特定 iPhone 型号，也不得退化为平台 Emoji；
  - 卡片标题统一表达「已信任设备」，每行不再重复信任状态；行尾「等待接入」状态与设备操作使用固定车道并保持垂直居中。删除操作使用具有稳定设备 ID 的语义 Button：静止态为透明底红色 `xmark`，采用标准中号组件保留大于图标的完整命中区，并提供包含设备名与身份前缀的 tooltip 与 accessibility label；点击后直接执行删除，不打开确认框；
  - 「重置全部配对」使用固定底栏危险操作，点击后直接异步清空可信设备并断开当前连接，不打开确认框；提交期间禁用重复命令，完成后反馈成功或失败；
  - 列表拥有独立纵向滚动区域并填充右侧可用高度；空列表显示首次连接指引。「自动接受可信设备」始终作为右侧卡片的固定底栏，不得随待机列表或直播控制内容移动。
- [ ] **AC-D-DEVICE-02（推流状态无缝切换）**：
  - Sender 建立推流后，待机设备列表和整个右侧「设备与连接」卡退出连接工作区；断开后恢复待机双栏，不保留两套并行的直播控制实现；
  - 唯一一处 `Live` 标记位于预览画面左上角（深色胶囊、绿色完成勾与 `Live` 文案）；顶栏设备名旁和其他区域不得重复「实时推流」或 `Live`；
  - 断开操作使用透明底红色 `xmark` +「断开」Button，保留标准命中区、tooltip 与 accessibility label，不显示填充危险按钮；点击后立即安全挂断并返回待机双栏，不打开确认框；
  - 身份、虚拟摄像头当前状态（包括「已随附 · 待激活」）、真实视频规格、可观测丢片、抖动和播放缓冲归属「连接详情」Popover；该 Popover 用于深入诊断，不用于收纳顶栏空间不足时被降级的帧率、延迟、码率或网络质量。
- [ ] **AC-D-NET-03（预览下方横向网络状态栏）**：
  - 网络状态从右侧辅助栏移到左侧主预览/待机卡的内部底部，4 项原生指标横向等分摊平（网络: 局域网可用 / 发现服务: 在线 / 延迟: 低 / 连接保护: 已验证），只使用主卡内部的顶部细分隔线，不得形成第二个独立长边框，也不得挤占右侧设备列表高度。
  - 四项分别使用 Wi-Fi、服务、活动、安全语义 Reicon，并使用「网络可用 / 发现在线 / 延迟低（或待测）/ 已验证（或等待连接）」等不重复图标语义的紧凑文案；值末尾使用完成、等待或异常状态图标，不得只依赖颜色圆点表达状态。每项保持单行并等宽展开，在 1440×900 默认与最小窗口下图标、文案和状态符号必须完整显示；更大界面缩放受压时才允许由所属项截断，不得推动整条状态栏横向溢出。
  - 待机主卡底部不得再并列一套重复的局域网监听与安全直连文案；监听地址由配对区的可复制地址承担，连接与安全摘要由横向状态区承担。

### 4.4 直播接收监视器与镜头控制面板
- [ ] **AC-D-LIVE-01**：推流时连接工作区整体切换为实时大屏视频流监视器，不再保留右侧设备栏。顶部单行工具栏下方的全部剩余空间用于 16:9 `VideoSurface`，画面必须在可用区域内取最大尺寸并保持为页面最大的视觉对象。Live 工具栏内容与预览画面在 Sidebar 展开态共用连接工作区的单一 `p_4` 外边距和首尾对齐线，不得分别叠加整层页面 padding；当窗口宽高关系使预览从宽度受限转为高度受限时（包括 Sidebar 折叠或 Windows 窗口最大化），预览舞台必须保持居中，并在顶部分隔线与窗口底部之间形成对称的 `p_4` 安全留白；不得以 Sidebar 是否折叠推断预览约束方向，画面也不得贴住顶栏分隔线或窗口边缘。顶栏与预览之间使用一条细分隔线。预览画面内部只允许左上角一处深色 `Live` 胶囊，胶囊以同一 `p_4` spacing token 避开预览圆弧，不得叠加设备名、分辨率、帧率、虚拟摄像头状态或等待提示。顶栏左侧为设备名与帧率、链路延迟、接收码率、网络质量的独立圆角方块（`radius`，不是全圆角胶囊）；分辨率选择、录像、「连接详情」与镜像、切换、修复、断开同为右侧带图标和文字的 outline 按钮，并保持可见。待机连接页与工作区标题行不得显示录像入口。
- [ ] **AC-D-LIVE-02（镜头与画面控制）**：
  - Live 顶栏右侧展开分辨率选择、录像、「连接详情」与 3 组真实可用的镜头控制按钮。分辨率选择使用与镜像/切换/修复相同的 outline 按钮样式，显示当前档位（`480p` / `720p` / `1080p`）并打开档位菜单：
    1. `mirror` Reicon + **`镜像`**（完整语义与辅助说明为「镜像翻转」，水平镜像翻转画面）；
    2. `switchCamera` Reicon + **`切换`**（完整语义与辅助说明为「镜头切换」，通过 `SWITCH_CAMERA` 让 Sender 根据实际活动镜头切换前/后摄，不由 Receiver 猜测当前朝向）；
    3. `repairFrame` Reicon + **`修复`**（完整语义与辅助说明为「画面修复」，请求 IDR 关键帧消除卡顿与花屏）。
  - 分辨率选择、录像、「连接详情」、三个镜头按钮与透明底红色 `xmark` +「断开」按钮保持同一行，并始终同时显示图标和单行文字；不得使用隐藏、折叠、改成 icon-only 或把分辨率收入「连接详情」的响应式降级。完整名称同时通过 tooltip 与 accessibility label 保留。
  - 录像只在连接成功后的 Live 顶栏出现，紧随分辨率选择，使用相同的 outline 按钮样式与「录像」短文案；菜单内分别选择原码流或处理后录像。待机连接页和工作区标题行不得再放置录像入口。录像生命周期仍由 Receiver 持有，离开 Live 或切换 Sidebar 页面不停止录像。
  - 未实现本地截图保存前不展示或伪装 `拍照截图` 控件。
- [ ] **AC-D-LIVE-03（桌面运行诊断）**：
  - Live 顶栏常驻显示实测帧率、链路延迟、接收码率和网络质量，分辨率选择也保持可见；这些一级状态不得收入「连接详情」。可观测视频分片丢失等深入指标放入「连接详情」；缺少首个 `ReceiverStats` 窗口时显示“— / 网络待测”，不得以 `0 ms / 0% / 极佳` 冒充实测结果；
  - 一级“网络”页提供网络与传输、媒体处理、输出链路三个分层区域，并以 1 秒为采样粒度保留最近 10 分钟有界历史摘要；历史必须在内存中限长，断流后仍可用于定位最近故障；
  - `packet_loss` 文案必须说明它是已决 AU 内确认缺失 fragment 的“可观测丢片”，不是 QUIC/网卡全链路精确丢包率；未完成双端时钟同步前 RTT 只能标为“链路延迟”；
  - 桌面不得展示尚未采集的 VCam RequestSample/fresh/cached/placeholder 数值；手机端不新增诊断页面或重复展示完整分层指标。
...
- [ ] **AC-D-NAV-01（4+2 极简导航体系）**：
  ...
  - 「关于」页在产品名下显示与 Windows 安装包相同的三字段产品版本。主次版本来自 workspace SemVer；编译环境提供 `PICOO_BUILD_NUMBER` 时用它替换补丁位，例如 `0.1.637`。
...
3. ❌ **图标 Emoji 化与样式分裂**：
   ...
   - 严禁使用带有 `← 返回` 文字的旧式导航条。
4. ❌ **断开失效与假死**：
   - 桌面端断开必须由 Live 工具栏单击直接执行，不得因确认框或跨实体回调失效而保留会话；移动端继续按 AC-M-LIVE-06 使用 Armed 二次点击；
