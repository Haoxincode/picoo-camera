...
- [ ] **AC-M-DISC-05**：局域网未发现设备时使用同一页面层级中的单一发现面板，不嵌套多层 Card；面板复用桌面端同源 MacBook 真机框资产表达 Receiver，并区分「正在搜索」与「未发现」状态。排查说明使用用户可执行的语言，提示保持电脑端 Picoo Camera 已打开、两端连接同一 Wi‑Fi；搜索中提供「停止搜索」，完成后提供「重新搜索附近电脑」，并始终保留一个清晰的「通过 IP 地址连接」次级动作，不要求用户理解服务发现协议。发现到 Receiver 后切换为原生设备行，再进入双端短码核对页。
