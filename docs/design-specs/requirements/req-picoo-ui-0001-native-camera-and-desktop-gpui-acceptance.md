...
### 4.4 直播接收监视器与镜头控制面板
- [ ] **AC-D-LIVE-01**：推流时连接工作区整体切换为实时大屏视频流监视器，不再保留右侧设备栏。顶部单行工具栏下方的全部剩余空间用于 16:9 `VideoSurface`，画面必须在可用区域内取最大尺寸并保持为页面最大的视觉对象。Live 工具栏内容与预览画面在 Sidebar 展开态共用连接工作区的单一 `p_4` 外边距和首尾对齐线，不得分别叠加整层页面 padding；Sidebar 折叠使预览从宽度受限转为高度受限时，预览舞台允许使用额外一层 `p_4` 安全留白并保持居中，画面不得贴住顶栏分隔线或窗口边缘。顶栏与预览之间使用一条细分隔线。预览画面内部只允许左上角一处深色 `Live` 胶囊，胶囊以同一 `p_4` spacing token 避开预览圆弧，不得叠加设备名、分辨率、帧率、虚拟摄像头状态或等待提示。顶栏左侧为设备名与帧率、链路延迟、接收码率、网络质量的独立圆角方块（`radius`，不是全圆角胶囊）；分辨率选择、「连接详情」与镜像、切换、修复、断开同为右侧带图标和文字的 outline 按钮，并保持可见。
