...
- [ ] **AC-M-DISC-05**：局域网未发现设备时使用同一页面层级中的克制空状态，不嵌套多层 Card；排查说明使用用户可执行的语言，提示保持电脑端 Picoo Camera 已打开、两端连接同一 Wi‑Fi，并提供一个清晰的「输入 IP 直连」次级动作，不要求用户理解服务发现协议。发现到 Receiver 后再进入双端短码核对页。
