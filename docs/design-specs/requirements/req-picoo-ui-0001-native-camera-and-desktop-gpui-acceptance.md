...
- [ ] **AC-D-TECH-03（gpui-component 与 Tailwind CSS 4.0 对齐）**：
  ...
  - 允许且必须保留经产品确认的原型覆盖项：默认窗口与最小窗口均为 1120×720、不记忆上次窗口尺寸、连接页使用可用宽度而不保留 HTML 的 1160px 上限、实时预览严格保持 16:9；窗口边界属于平台物理尺寸，可使用 `px(...)`。
...
- [ ] **AC-D-LAYOUT-01（主区域优先的稳定分栏）**：
  ...
  - 右侧只承载「设备与连接状态机」并填满连接工作区的可用高度。待机态允许在 21–24rem 内收缩且不消费窗口新增宽度；直播态收敛为 18rem 的紧凑检查器，把横向空间归还实时画面；
  - 连接工作区在 1120×720 最小窗口内不得依赖页面级滚动；可信设备列表独立持有纵向溢出，滚动条贴合列表所属面板尾边；分栏间距与内容 inset 使用统一 `rem` scale，不得以固定像素分别修正。
- [ ] **AC-D-HOST-02（主机识别卡片）**：
...
- [ ] **AC-D-ONBOARDING-03（开始使用与硬件拓扑）**：
  ...
  - 真机拓扑作为一个完整构图使用比例槽位随卡片等比收缩，并设置舒适最大宽度；在 1120px 最小窗口、Sidebar 展开态与界面缩放后，手机、无线波纹和电脑必须完整保留在拓扑卡片内，不得依赖裁掉溢出内容掩盖布局错误；
  - “自动接受可信设备”使用设置行的次级字号，不得与卡片标题争夺层级。

### 4.3 右侧设备连接状态机与左侧网络状态栏
- [ ] **AC-D-DEVICE-01（已信任与最近连接设备列表）**：
  - Receiver 不主动发现或连接 Sender。待机时设备卡只呈现本机可信设备存储中的已信任/最近连接设备；每条设备固定为名称、人类可读的最近连接时间（`今天` / `昨天` / `N 天前`）、可区分身份指纹前缀三行，等待/离线由尾部状态承担；24rem 舒适宽度与 1120×720 默认窗口下不得截断时间或指纹，不提供伪造的 `[ 连接 ]` 动作；
...
- [ ] **AC-D-DEVICE-02（推流状态无缝切换）**：
  ...
  - 原地切换为当前推流设备条目；直播状态只在卡片标题栏右上角保留一个绿色 `Live`，设备条目内部不得重复「实时推流」文案；
  - 断开操作使用透明底红色 `xmark` 图标 Button，保留标准命中区、tooltip、accessibility label 与确认框，不显示填充危险按钮；
  - 展开虚拟摄像头当前状态（包括「已随附 · 待激活」）、真实视频规格参数（H.264、720p30/1080p30、码率/电量）与 3 组已实现的镜头画面控制；直播内容独立纵向滚动，不得推动固定底栏越过卡片边框。
- [ ] **AC-D-NET-03（预览下方横向网络状态栏）**：
  ...
  - 四项分别使用 Wi-Fi、服务、活动、安全语义 Reicon，并使用「网络可用 / 发现在线 / 延迟低（或待测）/ 已验证（或等待连接）」等不重复图标语义的紧凑文案；值末尾使用完成、等待或异常状态图标，不得只依赖颜色圆点表达状态。每项保持单行并等宽展开，在 1120×720 默认与最小窗口下图标、文案和状态符号必须完整显示；更大界面缩放受压时才允许由所属项截断，不得推动整条状态栏横向溢出。
  - 待机主卡底部不得再并列一套重复的局域网监听与安全直连文案；监听地址由配对区的可复制地址承担，连接与安全摘要由横向状态区承担。

### 4.4 直播接收监视器与镜头控制面板
- [ ] **AC-D-LIVE-01**：推流时左侧大卡片自动切换为实时大屏视频流监视器（16:9 画幅、Canvas 动态推流渲染）。实时卡必须填满连接工作区高度并与右侧「设备与连接」卡的上下边框对齐；16:9 画面在主卡剩余空间内取最大尺寸并保持为页面最大的视觉对象。预览画面内部不叠加设备名、分辨率、帧率、虚拟摄像头状态、等待提示或 `LIVE` 标记；链路指标保留在画面之外的摘要栏，虚拟摄像头状态归属右侧设备栏。
- [ ] **AC-D-LIVE-02（镜头与画面控制）**：
  - 右侧 Box 1 展开 3 组真实可用的镜头控制按钮：
    1. `mirror` Reicon + **`镜像`**（完整语义与辅助说明为「镜像翻转」，水平镜像翻转画面）；
    2. `switchCamera` Reicon + **`切换`**（完整语义与辅助说明为「镜头切换」，前置 / 后置主摄 / 超广角）；
    3. `repairFrame` Reicon + **`修复`**（完整语义与辅助说明为「画面修复」，请求 IDR 关键帧消除卡顿与花屏）。
  - 三个按钮在 18rem 直播检查器内保持同一行、等宽和单行文案；按钮及所在控制行必须具备 `min_w_0` 收缩链，图标与文字不得越过右侧卡片边框，完整名称通过 tooltip 与 accessibility label 保留。
  - 未实现本地截图保存前不展示或伪装 `拍照截图` 控件。
- [ ] **AC-D-LIVE-03（桌面运行诊断）**：
  - 直播监视器摘要显示目标/实测帧率、接收码率、链路延迟、可观测视频分片丢失和网络质量；缺少首个 `ReceiverStats` 窗口时显示“等待样本”，不得以 `0 ms / 0% / 极佳` 冒充实测结果；
