...
- [ ] **AC-D-DEVICE-02（推流状态无缝切换）**：
  ...
  - 顶栏设备名右侧只保留一个绿色圆点与绿色 `Live`，画面内部和其他区域不得重复「实时推流」或 `Live`；
...
### 4.4 直播接收监视器与镜头控制面板
- [ ] **AC-D-LIVE-01**：推流时连接工作区整体切换为实时大屏视频流监视器，不再保留右侧设备栏。顶部单行工具栏下方的全部剩余空间用于 16:9 `VideoSurface`，画面必须在可用区域内取最大尺寸并保持为页面最大的视觉对象。预览画面内部不叠加设备名、分辨率、帧率、虚拟摄像头状态、等待提示或 `LIVE` 标记；全部状态和动作位于画面之外。
- [ ] **AC-D-LIVE-02（镜头与画面控制）**：
  - Live 顶栏右侧展开 3 组真实可用的镜头控制按钮：
  ...
  - 三个按钮与透明底红色 `xmark` +「断开」按钮保持同一行，并始终同时显示图标和单行文字；不得使用隐藏、折叠、改成 icon-only 或移入「连接详情」的响应式降级。完整名称同时通过 tooltip 与 accessibility label 保留。
