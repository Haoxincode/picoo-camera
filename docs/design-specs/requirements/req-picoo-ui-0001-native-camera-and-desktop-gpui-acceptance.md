...
- [ ] **AC-M-LIVE-03（取景器手势与安全框）**：
  - 电脑端输出固定为横向 `16:9`，不提供 `9:16` 输出切换。取景画面正中常驻四角式 `16:9 电脑端输出范围`，其几何必须与 Receiver/Virtual Camera 最终可见范围一致；
  - 手机横持时框体尽量利用完整取景画面；手机竖持时框体显示中央横向裁切区域，并提示 `建议横屏使用，电脑端仅显示框内区域`；
