...
- [ ] **AC-M-DISC-02**：发现状态位于设备列表标题附近，使用语义 `discovering` Icon、品牌色动效和明确文案；发现完成/监听中使用稳定状态而非永久环境装饰。主流程只描述“搜索同一 Wi‑Fi 下的电脑”等用户任务，不展示 Bonjour、NSD、mDNS 或 `_picoocam._udp` 等实现标识；这些信息仅进入诊断数据、日志和系统配置。Reduced Motion 下停止呼吸缩放但保留文案与颜色变化。
...
- [ ] **AC-M-DISC-05**：局域网未发现设备时使用同一页面层级中的单一发现面板，不嵌套多层 Card；面板复用桌面端同源 MacBook 真机框资产表达 Receiver，并区分「等待发现权限」「正在搜索」与「未发现」状态。发现权限未授予时不得在面板上方追加独立权限 Card 或把主内容整体下推，而是在同一面板内切换说明和主操作。排查说明使用用户可执行的语言，提示保持电脑端 Picoo Camera 已打开、两端连接同一 Wi‑Fi；搜索中提供「停止搜索」，完成后提供「重新搜索附近电脑」，并始终保留一个清晰的「通过 IP 地址连接」次级动作，不要求用户理解服务发现协议。发现到 Receiver 后切换为原生设备行，再进入双端短码核对页。

### 3.2 手动 IP 直连抽屉（Screen 2 - Manual Connect Sheet）
- [ ] **AC-M-MANUAL-01**：点击主页动作升起原生 Sheet，只接收局域网 IPv4 与端口（如 `192.168.1.108:4433`）；地址使用四段数字输入格，点号和冒号由界面固定呈现，端口默认预填 `4433` 且可编辑。每段达到可确定边界后自动前进，键盘 Next 可前进较短段；粘贴完整 `IP:端口` 时自动分配到各输入格。通过 mDNS 选择设备时不展示此输入。紧凑窗口可使用半屏，键盘或较大文字需要更多空间时允许扩展到大 Detent/完整页面。原生 Compose/SwiftUI 输入与焦点 API 已满足该单一用途，不引入第三方 OTP、地址或表单组件依赖。
...
- [ ] **AC-M-LIVE-01（顶部 HUD）**：
  - 顶部居中两行显示连接状态与实时遥测。第一行为品牌蓝连接点 + `电脑设备名 已连接`；第二行为 RTT、实时码率、分辨率和帧率（如 `63ms · 1.9 Mbps · 720p / 30fps`）；
