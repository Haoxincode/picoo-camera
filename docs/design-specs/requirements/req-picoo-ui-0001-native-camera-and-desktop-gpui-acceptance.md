...
- [ ] **AC-D-DEVICE-01（已信任与最近连接设备列表）**：
  ...
  - 卡片标题统一表达「已信任设备」，每行不再重复信任状态；行尾「等待接入」状态与设备操作使用固定车道并保持垂直居中。删除操作使用具有稳定设备 ID 的语义 Button：静止态为透明底红色 `xmark`，采用标准中号组件保留大于图标的完整命中区，并提供 tooltip 与 accessibility label；点击后必须打开命名具体设备及后果的删除确认框；
