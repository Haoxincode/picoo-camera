...
- [ ] **AC-D-DEVICE-01（已信任与最近连接设备列表）**：
  ...
  - 同名条目必须显示至少 8 位且发生碰撞时自动延长的公钥指纹前缀，并按最近连接时间稳定排序；未知新身份完成配对后如存在同名历史身份，使用不可点外部关闭的 AlertDialog 逐条列出候选指纹并明确提供“仅保留当前身份 / 保留全部”；提示必须等待已有 Dialog 关闭且不得抢焦点，可信重连不得触发；替换只执行所展示的 revision 快照，候选失效或持久化失败时保留 Dialog 并显示错误；
