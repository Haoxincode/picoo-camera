...
- [ ] **AC-D-LIVE-02（镜头与画面控制）**：
  ...
  - 分辨率选择、「连接详情」、三个镜头按钮与透明底红色 `xmark` +「断开」按钮保持同一行，并始终同时显示图标和单行文字；不得使用隐藏、折叠、改成 icon-only 或把分辨率收入「连接详情」的响应式降级。完整名称同时通过 tooltip 与 accessibility label 保留。
