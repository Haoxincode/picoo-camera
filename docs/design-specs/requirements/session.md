...
| REQ-PICOO-SESSION-005 | implemented | PRD §21 | 2h 长稳无崩溃、内存不增长 | `soak_harness_smoke_five_seconds`（CI）；`scripts/soak_loopback.sh` + ignored 长 soak（已跑 90s/600s RSS 平台；`SOAK_SECONDS=7200` 做 2h） |
