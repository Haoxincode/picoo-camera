...
| REQ-PICOO-VCAM-002 | planned | ARCH-PICOO-VCAM-001 | MFCreateVirtualCamera + 独立 Rust IMFMediaSource DLL | 普通启动只通过无注册表写入能力的 `start_registered` 启动已安装设备，Session 生命周期由桌面进程持有；`windows-rs` 实现 COM/MF；`MENewStream`/`MEUpdatedStream`；480p/720p/1080p NV12；输出格式跟随 Shared Frame Ring；`windows-latest` Cargo 产出 `PicooVirtualCameraSource.dll`；Windows bundle export/load smoke；真机枚举仍待 |
...
| REQ-PICOO-VCAM-004 | implemented | PUC-001 | 安装器注册 COM/MF 组件与卸载清理；系统级修复只由安装器或用户显式操作触发 | WiX 声明式 COM 注册表 + FirewallException QUIC(4433) + 安装/卸载时 WixQuietExec 调用 `--register-vcam --no-wait` / `--unregister-vcam`；设置页显式修复继续使用可写注册路径，普通启动与状态检测不写 HKLM、不启动 `reg.exe`；CLSID 三方同步；`validate_wix_scaffold.sh`；CI 钉扎 WiX 5.0.2；MSI 真机仍待 |
