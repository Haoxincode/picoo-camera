...
| REQ-PICOO-PRIVACY-003 | implemented | PUC-007 | 日志不含视频帧；IP/设备名/指纹可脱敏 | `picoo-diagnostics` 默认脱敏 + 导出/CLI/设置页指纹脱敏测试 |
| REQ-PICOO-PRIVACY-004 | implemented | PRD §21 | QUIC/TLS 加密，抓包无明文控制/视频 | 控制+视频经 quiche ALPN `picoocam/1`（发现 mDNS/QR 明文属设计） |
| REQ-PICOO-PRIVACY-005 | implemented | PUC-007 | 断网外网后 LAN 功能仍可用 | 无云依赖；发现+传输均为局域网（缺自动化 WAN 断网用例，靠架构审查） |
