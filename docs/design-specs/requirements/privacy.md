...
| REQ-PICOO-PRIVACY-003 | implemented | PUC-007 | 日志不含视频帧；IP/设备名/指纹可脱敏 | `picoo-diagnostics` 默认脱敏；session 计数 role-neutral；Android/桌面导出；`diagnostics_export` 桌面路径单测 |
| REQ-PICOO-PRIVACY-004 | implemented | PRD §21 | QUIC/TLS 加密，抓包无明文控制/视频 | 控制+视频经 Quinn/Rustls ALPN `picoocam/1`；mDNS 广播仅含公开白名单字段 |
