...
| REQ-PICOO-PRIVACY-003 | implemented | PUC-007 | 日志不含视频帧；IP/设备名/指纹可脱敏 | `picoo-diagnostics` 默认脱敏；session 计数 role-neutral（`access_units`/`packets`）；Android/桌面导出含脱敏 hosts |
