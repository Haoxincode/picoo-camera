...
Windows生产接线复用现有EncodedWriter/RecordingWorker和桌面原生目录选择，不新增平台专属业务状态机。两平台合成bundle验证共用xtask入口，独占输出目录使用仓库已有tempfile 3，替换macOS专属mktemp进程；Windows CI保留两套八组合bundle供独立检查。新生产回调和bundle结果须由新CI验证，前述研究矩阵不替代生产路径证据。
